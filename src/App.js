/**
 * PV Pathfinder - Academic Planning Assistant for Prairie View A&M University
 * AI-powered course recommendations, semester roadmap, and transcript import
 */
...
function Dashboard({ onNavigate, profileData }) {
  ...
  /** Calculate student GPA using standard 4.0 scale */
  const calculateGPA = () => {
    if (!profileData?.courses || profileData.courses.length === 0) return 0;
    
    const gradePoints = { 'A': 4.0, 'B': 3.0, 'C': 2.0, 'D': 1.0, 'F': 0.0 }; // Standard 4.0 scale
    let totalPoints = 0;
    let totalCredits = 0;
...
function CourseCatalog({ profileData, courseCatalog, isLoadingCourses }) {
  ...
  /** Check if student has met course prerequisites */
...
function ProfileForm({ onSaveProfile, existingProfile, showToast, onTempUpdate, courseCatalog, onClearAllData }) {
  ...
  /** Validate profile form before saving */
  ...
  // Ctrl+S keyboard shortcut to save profile
  React.useEffect(() => {
        ...
        <div className="course-list">
          {courses.length === 0 ? (
            ...
            (() => {
              ...
              /** Convert semester to sortable number (e.g., "Fall 2024" -> 20242) */
              const semesterOrder = (sem) => {
                const match = sem.match(/(Fall|Spring|Summer)\s+(\d{4})/);
                if (!match) return 0;
                const [, season, year] = match;
                const seasonValue = { Spring: 0, Summer: 1, Fall: 2 }[season] || 0; // Order within year
                return parseInt(year) * 10 + seasonValue; // e.g., Fall 2024 = 20242
              };
